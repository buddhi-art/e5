...
const MIN_WORK_MS = MIN_WORK_MINUTES * 60 * 1000

function calculateRemainingMs(checkInTime: string | null, nowMs: number) {
  if (!checkInTime) return 0

  const elapsed = nowMs - new Date(checkInTime).getTime()
  return Math.max(0, MIN_WORK_MS - elapsed)
}
...
export function AttendanceForm({ hasCheckedIn, hasCheckedOut, checkInTime }: AttendanceFormProps) {
  ...
  useEffect(() => {
    if (!localCheckInTime || hasCheckedOut || !localCheckedIn) return undefined
  ...
  }, [hasCheckedOut, localCheckInTime, localCheckedIn])

  const effectiveCheckInTime = localCheckInTime || checkInTime
  const remainingMs = localCheckedIn && !hasCheckedOut
    ? calculateRemainingMs(effectiveCheckInTime, nowMs)
    : 0
  ...
  const progressPercent = effectiveCheckInTime
    ? Math.min(100, ((MIN_WORK_MS - remainingMs) / MIN_WORK_MS) * 100)
    : 0
  ...
  async function handleCheckIn() {
    ...
    try {
      ...
      setLocalCheckInTime(result.checkInTime || new Date().toISOString())
      setNowMs(Date.now())
            ...
            <div className="space-y-4">
              {effectiveCheckInTime && (
                <div className="p-3 bg-m3-success-subtle rounded-lg border border-m3-success/50">
                  ...
                  <div className="text-lg font-bold text-m3-success mt-1">{formatTime(effectiveCheckInTime)}</div>
                </div>
              )}
              ...
              <Button
                ...
                size="lg"
                className={`w-full font-semibold text-lg h-14 shadow-lg border-none disabled:opacity-50 disabled:cursor-not-allowed ${isCheckoutLocked
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="p-3 bg-m3-success-subtle rounded-lg border border-m3-success/50">
                  ...
                  <div className="text-base font-bold text-m3-success mt-1">
                    {effectiveCheckInTime ? formatTime(effectiveCheckInTime) : '—'}
                  </div>
                </div>
