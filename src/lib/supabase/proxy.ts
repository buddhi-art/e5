...
export async function updateSession(request: NextRequest) {
    ...
    const supabase = createServerClient(
        ...
        process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY!,
        {
            cookies: {
                ...
                setAll(cookiesToSet) {
                    // Set cookies only on the response, preserving security flags
                    // (httpOnly, secure, sameSite) carried in `options`. Do NOT call the
                    // bare `request.cookies.set(name, value)` first — it drops `options`
                    // and can leak session cookies without security attributes.
    ...
    // Logged in and on login page or the neutral base path → redirect to portal.
    // login() redirects to '/' after auth and relies on this to reach the portal.
    if (path === '/login' || path === '/') {
        try {
