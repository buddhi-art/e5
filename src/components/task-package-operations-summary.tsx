...
export function TaskPackageOperationsSummary({ projectId, phase }: { projectId: string, phase: string }) {
  ...
  const names = people.map((id: string) => employees.find((employee: any) => employee.id === id)?.full_name).filter(Boolean)
  ...
  const hasContent = location || date || start || end || names.length > 0 || (phase === 'Phase 2' ? logistics.revision_count > 0 : data.deliverables?.length > 0)
  if (!hasContent) return null

  return <div className="mt-4 rounded-lg border border-outline-variant/60 bg-surface-container-low p-3"><h4 className="flex items-center gap-2 text-xs font-semibold text-foreground">{phase === 'Phase 2' ? <Truck className="h-4 w-4 text-primary" /> : <Video className="h-4 w-4 text-primary" />}{phase === 'Phase 2' ? 'Videography & on-site logistics' : 'Editing & deliverable hub'}</h4><div className="mt-2 grid grid-cols-1 gap-2 text-xs text-on-surface-variant sm:grid-cols-2">{location && <span className="flex min-w-0 items-center gap-1.5"><MapPin className="h-3.5 w-3.5 shrink-0 text-primary" /><span className="truncate">{location}</span></span>}{date && <span className="flex items-center gap-1.5"><Calendar className="h-3.5 w-3.5 text-primary" />{date}</span>}{(start || end) && <span className="flex items-center gap-1.5"><Clock className="h-3.5 w-3.5 text-primary" />{start || '?'} – {end || '?'}</span>}{names.length > 0 && <span className="flex min-w-0 items-center gap-1.5"><CheckCircle2 className="h-3.5 w-3.5 shrink-0 text-primary" /><span className="truncate">{names.join(', ')}</span></span>}{phase === 'Phase 2' && logistics.revision_count > 0 && <span>{logistics.revision_count} extra site visit{logistics.revision_count === 1 ? '' : 's'}</span>}{phase === 'Phase 3' && data.deliverables?.length > 0 && <span>{data.deliverables.length} deliverable{data.deliverables.length === 1 ? '' : 's'}</span>}</div></div>
}
