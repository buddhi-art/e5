-- Create a function and trigger to auto-update a parent task's status based on its subtasks
...
DROP TRIGGER IF EXISTS trg_update_task_status_from_subtasks ON subtasks;
CREATE TRIGGER trg_update_task_status_from_subtasks
AFTER INSERT OR UPDATE OF is_completed OR DELETE
ON subtasks
FOR EACH ROW
EXECUTE FUNCTION public.update_task_status_on_subtask_change();
