...
-- 3. Indexes for Performance (only create if the tables exist)
DO $$ BEGIN
  ...
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'invoices') THEN
    CREATE INDEX IF NOT EXISTS idx_invoices_status_issue_date ON public.invoices(status, issue_date);
  END IF;
END $$;
